...
export class GraphQLQueryBuilder {
  ...
  private resolveParamType = (param: { key?: string; option: ParamOption }): string => {
    ...
    if (typeof option === 'boolean' || typeof option === 'number') {
      return `${_key}${option}`
    }
  ...
  private buildParams = (
    ...
    if (Array.isArray(options)) {
      output = `${options
        .filter((option) => !isEmpty(option) && !isNil(option))
        .map((option) => this.resolveParamType({ option }))
        .join(', ')}`
    } else {
  ...
  private buildVariables = (options: VariableOptions): string => {
    ...
    Object.entries(options)
      .filter(([, option]) => !isEmpty(option.value) && !isNil(option.value))
      .forEach(([key, option]) => {
