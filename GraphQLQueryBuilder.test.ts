...
describe('GraphQLQueryBuilder', () => {
  ...
  describe('Mutation requests', () => {
    ...
    it('should build mutation with multiple operation, complex params and alias', async () => {
      ...
      expect(mutation).toMatchObject({
        request: `mutation { alias1: createCustomer(input: { data: { firstName: "fname" lastName: "lname" dateOfBirth: "${dateString}" } source: { name: "latam" id: "external-id1" createdAt: "${dateString}" } }) { id } alias2: createCustomer(input: { data: { firstName: "fname" lastName: "lname" gender: "F" } source: { name: "latam" id: "external-id2" createdAt: "${dateString}" } }) { id } alias3: updateCustomer(input: { id: "internal-uuid3" data: { dateOfBirth: "${dateString}" } source: { name: "latam" updatedAt: "${dateString}" } }) { id } alias4: updateCustomer(input: { id: "internal-uuid4" data: { gender: "M" dateOfBirth: null addresses: { create: [ { data: { lat: 1.23 lng: 3.123 } source: { name: "latam" id: "external-address-id1" createdAt: "${dateString}" } }, { data: { lat: 1.23 lng: 3.123 } source: { name: "latam" id: "external-address-id2" createdAt: "${dateString}" } } ] } } source: { name: "latam" updatedAt: "${dateString}" } }) { id } }`,
        variables: {},
      });
    });
        ...
        },
        {
          ...
          variables: {
            input4: {
              ...
              value: {
                id: 'internal-uuid4',
                data: { gender: 'M', dateOfBirth: null },
                source: { name: 'latam', updatedAt: date },
              },
            },
          },
